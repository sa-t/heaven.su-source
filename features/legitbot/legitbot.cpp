...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (target_switch_delay > m_globals()->m_realtime)
	{
		...
		if (((cmd->m_buttons & IN_ATTACK) || key_binds::get().get_key_bind_state(0)) && !g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs_type)
			do_rcs(cmd);
	...
	if (!(cmd->m_buttons & IN_ATTACK) && !key_binds::get().get_key_bind_state(0))
	{
		autofire_delay = 0;
...
void legit_bot::find_best_point(CUserCmd* cmd, float fov_v)
{
	...
	else if (g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 1) 
	{
		float distance = 0.f;
		float fov = FLT_MAX;
		Vector ang = Vector(0, 0, 0);
	...
	else if (g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 2) 
	{
