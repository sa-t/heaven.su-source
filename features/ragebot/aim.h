...
class aim : public singleton <aim>
{
	...
	bool detect_hit_another_hitbox_due_to_spread(int client_hitbox, Vector aim_point);
	void fire(CUserCmd* cmd);
	void build_seed_table();
	bool calculate_hitchance(int& final_hitchance);
	bool hitbox_intersection_fixed(player_t* e, matrix3x4_t* matrix, int hitbox, const Vector& start, const Vector& end, float* safe);
