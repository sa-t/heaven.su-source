...
void antiaim::create_move(CUserCmd* m_pcmd)
{
	...
	if (!g_cfg.misc.fast_stop && (!g_ctx.globals.weapon->is_grenade() || g_cfg.esp.on_click && !(m_pcmd->m_buttons & IN_ATTACK) && !(m_pcmd->m_buttons & IN_ATTACK2)) && engineprediction::get().backup_data.velocity.Length2D() <= 20.0f)
	{
		auto speed = 1.01f;
...
float antiaim::get_pitch(CUserCmd* m_pcmd)
{
	...
	{
	case 1:
		m_pcmd->m_viewangles.x = 89.0f;
		break;
	case 2:
		m_pcmd->m_viewangles.x = -89.0f;
		break;
	case 3:
		m_pcmd->m_viewangles.x = 0.0f;
		break;
	}
...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	{
		if (base_angle != m_pcmd->m_viewangles.y + 180.0f)
	...
	{
	case 1:
		yaw_angle = invert_jitter ? -(float)g_cfg.antiaim.range / 2 : (float)g_cfg.antiaim.range / 2;
		break;
	case 2:
	{
		if (flip)
