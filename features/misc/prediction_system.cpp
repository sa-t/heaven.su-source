...
{
	engine_prediction::predict(cmd, player);
}
...
namespace engine_prediction
{
	...
	void predict(CUserCmd* cmd, player_t* player)
	{
		...
		static auto using_standard_weapons_in_vehicle = reinterpret_cast <bool(__thiscall*)(void*)>(util::FindSignature(crypt_str("client.dll"), crypt_str("56 57 8B F9 8B ? ? ? ? ? 83 FA FF 74 43")));
	...
	void patch_attack_packet(CUserCmd* cmd, bool restore)
	{
		...
		C_BaseViewModel* view_model = reinterpret_cast<C_BaseViewModel*>(m_entitylist()->GetClientEntityFromHandle((CBaseHandle)g_ctx.local()->m_hViewModel()));
		if (!view_model)
			return;
	...
	void ProcessInterpolation(bool bPostFrame, player_t* player, CUserCmd* cmd)
	{
		...
		if (!bPostFrame)
		{
			...
			int m_iTickCount = g_ctx.globals.fakeducking ? (14 - m_clientstate()->iChokedCommands) : m_globals()->m_tickcount;	  // ������ 14 �� ������ �������������� choked commands
			g_ctx.local()->m_nFinalPredictedTick() = m_iTickCount + TIME_TO_TICKS(outgoing + ingoing);		// outgoing ������ ������ ��� chokedcommands, � ingoing ������ ��� tickcount
			return;
		}
