...
void playeresp::draw_health(player_t* m_entity, const Box& box, const HPInfo& hpbox)
{
	...
	render::get().rect(box.x - 6, box.y - 1, 4, box.h + 2, outline_color);
	...
	render::get().rect_filled(n_box.x, n_box.y - 1, 2, n_box.h + 2, inner_back_color);
	render::get().rect_filled(n_box.x, n_box.y + offset, 2, bar_height, color);

	auto height = n_box.h - hpbox.hp * n_box.h / 100;

	if (hpbox.hp < 100)
		render::get().text(fonts[FLAGS], n_box.x + 1, n_box.y + offset, text_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, std::to_string(hpbox.hp).c_str());
}
...
bool playeresp::draw_ammobar(player_t* m_entity, const Box& box)
{
	...
	if (weapon->m_iClip1() != weapon_info->iMaxClip1 && !reloading)
		render::get().text(fonts[FLAGS], n_box.x + bar_width, n_box.y + 1, text_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, std::to_string(ammo).c_str());
...
void playeresp::draw_name(player_t* m_entity, const Box& box)
{
	...
	if (m_engine()->GetPlayerInfo(m_entity->EntIndex(), &player_info))
	{
		...
		render::get().text(fonts[NAME], box.x + box.w / 2, box.y - 13, color, HFONT_CENTERED_X, name);
	}
}
...
void playeresp::draw_weapon(player_t* m_entity, const Box& box, bool space)
{
	...
	{
		render::get().text(fonts[FLAGS], box.x + box.w / 2, pos, color, HFONT_CENTERED_X, weapon->get_name().c_str());
		pos += 11;
	}
...
void playeresp::draw_flags(player_t* e, const Box& box)
{
	...
	if (g_cfg.player.type[type].flags[FLAGS_MONEY])
	{
		...
		render::get().text(fonts[FLAGS], _x, _y, color, HFONT_CENTERED_NONE, "%i$", e->m_iAccount());
		_y += 8;
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_ARMOR])
	{
		...
		if (kevlar)
		{
			render::get().text(fonts[FLAGS], _x, _y, color, HFONT_CENTERED_NONE, text.c_str());
			_y += 8;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_KIT] && e->m_bHasDefuser())
	{
		...
		render::get().text(fonts[FLAGS], _x, _y, color, HFONT_CENTERED_NONE, "KIT");
		_y += 8;
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_SCOPED])
	{
		...
		if (scoped)
		{
			...
			render::get().text(fonts[FLAGS], _x, _y, color, HFONT_CENTERED_NONE, "ZOOM");
			_y += 8;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_FAKEDUCKING])
	{
		...
		if (animstate)
		{
			auto fakeducking = [&]() -> bool
			{
				...
				if (animstate->m_flAnimDuckAmount) //-V550
				{
					if (animstate->m_flAnimDuckAmount < 0.9f && animstate->m_flAnimDuckAmount > 0.5f) //-V550
					{
			...
			if (g_cfg.player.type[type].flags[FLAGS_HIT] && weapon->is_non_aim())
			{
				auto color = e->IsDormant() ? Color(255, 255, 255, 255) : Color(255, 255, 255);
				color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()]);

				render::get().text(fonts[FLAGS], _x, _y, color, HFONT_CENTERED_NONE, "HIT");
				_y += 8;
			}

			if (fakeducking() && e->m_fFlags() & FL_ONGROUND && !animstate->m_bLanding)
			{
				auto color = e->IsDormant() ? Color(130, 130, 130, 130) : Color(215, 20, 20);
				color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()]);

				render::get().text(fonts[FLAGS], _x, _y, color, HFONT_CENTERED_NONE, "FD");
				_y += 8;
			}
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_C4] && e->EntIndex() == g_ctx.globals.bomb_carrier)
	{
		...
		render::get().text(fonts[FLAGS1], _x, _y, color, HFONT_CENTERED_NONE, "BOMB");
		_y += 8;
	}
}
