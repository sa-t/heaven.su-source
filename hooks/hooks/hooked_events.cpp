	...
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot && aim::get().last_target[current_shot->last_target].record.player->valid(true, false))
			{
				...
				trace_t trace, trace_zero, trace_first, trace_second, trace_low_first, trace_low_second, trace_low_first1, trace_low_second1;
				Ray_t ray;
				...
				if (aim::get().last_target[current_shot->last_target].data.point.safe)
				{
					...
					memcpy(aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().Base(), aim::get().last_target[current_shot->last_target].record.matrixes_data.first, aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().Count() * sizeof(matrix3x4_t));
					...
					memcpy(aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().Base(), aim::get().last_target[current_shot->last_target].record.matrixes_data.second, aim::get().last_target[current_shot->last_target].record.player->m_CachedBoneData().Count() * sizeof(matrix3x4_t));
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			if (g_cfg.esp.hitsound && g_cfg.player.enable)
				m_surface()->PlaySound_(crypt_str("hitsound.wav"));
			...
			otheresp::get().damage_marker[user_id].hurt_color = g_cfg.esp.damage_marker_color;
