...
void __fastcall hooks::hooked_runcommand(void* ecx, void* edx, player_t* player, CUserCmd* m_pcmd, IMoveHelper* move_helper)
{
	...
	if (m_pcmd->m_tickcount >= (m_globals()->m_tickcount + g_ctx.globals.tickrate + 8))
	{
...
void __vectorcall hooks::hooked_clmove(float flFrametime, bool bFinalTick)
{
	...
	((CLMove_t)original_clmove)(flFrametime, bFinalTick);
	...
	if (g_ctx.globals.should_recharge && g_ctx.globals.tocharge < g_ctx.globals.tochargeamount)
	{
		g_ctx.send_packet = false;
		g_ctx.globals.tocharge++;
		g_ctx.globals.ticks_allowed = g_ctx.globals.tocharge;
		g_ctx.local()->m_nTickBase()++;
		if (g_ctx.globals.tocharge <= 0) {
			g_ctx.local()->m_nTickBase()--;
		}
		return;
	}
	...
	{
		if (g_ctx.local() && g_ctx.local()->is_alive() && g_ctx.available() && m_clientstate() && m_clientstate()->pNetChannel)
		{
			auto net_channel = m_clientstate()->pNetChannel;
			if (net_channel->m_nChokedPackets > 0 && !(net_channel->m_nChokedPackets % 4))
			{
				const int32_t backup_choke = net_channel->m_nChokedPackets;
				const int32_t iSequenceNumber = net_channel->m_nOutSequenceNr;
				...
				net_channel->m_nOutSequenceNr = iSequenceNumber;
				net_channel->m_nChokedPackets = backup_choke;
			}
		}
	}
	while (g_ctx.globals.shift_ticks > 0)
	{
		g_ctx.globals.shifting = true;
		g_ctx.send_packet = g_ctx.globals.shift_ticks == 1;
		g_ctx.globals.shift_ticks--;
		g_ctx.globals.tocharge--;
		((CLMove_t)original_clmove)(flFrametime, g_ctx.send_packet);
	}
	g_ctx.globals.shifting = false;
	if (g_ctx.globals.should_recharge)
		g_ctx.globals.should_recharge = false;
}
...
next_cmd:
	...
	to_cmd.m_tickcount += g_ctx.globals.tickrate * 2;
