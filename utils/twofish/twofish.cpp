...
uint8_t* encrypt_string(std::string str, std::string key, size_t& size)
{
    ...
    twofish256 twofish;
    twofish256::user_key_t user_key;

    for (auto i = 0; i < KEY_SIZE; i++)
        user_key.at(i) = key[i];

    twofish.make_session_key(user_key);
    str.resize(str.size() - str.size() % BLOCK_SIZE + BLOCK_SIZE);

    size = str.size() / BLOCK_SIZE;
    std::vector <twofish256::block_t> blocks(size);

    for (auto i = 0; i < size; i++)
        for (auto j = 0; j < BLOCK_SIZE; j++)
            blocks.at(i).at(j) = str.at(i * BLOCK_SIZE + j);

    for (auto& block : blocks)
        block = twofish.encrypt(block);

    auto encrypted_str = new uint8_t[size * BLOCK_SIZE];

    for (auto i = 0; i < size; i++)
        for (auto j = 0; j < BLOCK_SIZE; j++)
            encrypted_str[i * BLOCK_SIZE + j] = blocks.at(i).at(j);
...
std::string decrypt_string(uint8_t* str, std::string key, size_t size)
{
    ...
    for (auto i = 0; i < size; i++)
        for (auto j = 0; j < BLOCK_SIZE; j++)
            blocks.at(i).at(j) = str[i * BLOCK_SIZE + j];

    for (auto& block : blocks)
        block = twofish.decrypt(block);

    std::string decrypted_str;

    for (auto i = 0; i < size; i++)
        for (auto j = 0; j < BLOCK_SIZE; j++)
            decrypted_str.push_back(blocks.at(i).at(j));

    if (decrypted_str.empty())
        return decrypted_str;

    while (decrypted_str.at(decrypted_str.size() - 1) == '\0')
