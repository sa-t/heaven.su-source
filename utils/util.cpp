...
namespace util
{
	...
	bool get_backtrack_matrix(player_t* e, matrix3x4_t* matrix)
	{
		...
		for (auto record = records->rbegin(); record != records->rend(); ++record)
		{
			...
			memcpy(result, record->matrixes_data.zero, MAXSTUDIOBONES * sizeof(matrix3x4_t));
			memcpy(result, record->matrixes_data.first, MAXSTUDIOBONES * sizeof(matrix3x4_t));
			memcpy(result, record->matrixes_data.second, MAXSTUDIOBONES * sizeof(matrix3x4_t));

			for (auto j = 0; j < MAXSTUDIOBONES; j++)
			{
				auto matrix_delta = math::matrix_get_origin(record->matrixes_data.main[j]) - record->origin;
				auto matrix_delta1 = math::matrix_get_origin(record->matrixes_data.second[j]) - record->origin;
				auto matrix_delta2 = math::matrix_get_origin(record->matrixes_data.zero[j]) - record->origin;
				auto matrix_delta3 = math::matrix_get_origin(record->matrixes_data.first[j]) - record->origin;
				math::matrix_set_origin(matrix_delta + lerp, result[j]);
				math::matrix_set_origin(matrix_delta1 + lerp, result[j]);
				math::matrix_set_origin(matrix_delta2 + lerp, result[j]);
				math::matrix_set_origin(matrix_delta3 + lerp, result[j]);
			}
