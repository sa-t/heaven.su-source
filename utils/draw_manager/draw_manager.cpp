...
void render::Draw3DCircleGradient(const Vector& origin, float radius, Color color, float alpha_multiplier)
{
	static auto prevScreenPos = ZERO; //-V656
	...
	for (int i = 1; i < radius; i++) {
		if (rad > 2.f)
			Draw3DFilledCircleTest(origin, round(rad), Color(color.r(), color.g(), color.b(), (int)(i * alpha_multiplier)));
