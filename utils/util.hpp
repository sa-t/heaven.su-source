...
namespace util
{
	...
	inline float csgo_armor(float damage, int armor_value) 
	{
	...
	static Type hook_manual(uintptr_t* vftable, uint32_t index, Type fnNew) 
	{
		...
		VirtualProtect((void*)(vftable + index * sizeof(Type)), sizeof(Type), PAGE_EXECUTE_READWRITE, &OldProtect);  
		vftable[index] = (uintptr_t)fnNew;  
		VirtualProtect((void*)(vftable + index * sizeof(Type)), sizeof(Type), OldProtect, &OldProtect);  
