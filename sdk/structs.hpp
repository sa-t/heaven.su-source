...
public:
    ...
    //NETVAR(int, m_angVisualAngles, crypt_str("CBasePlayer"), crypt_str("m_angVisualAngles"));
    ...
    int& m_nBody()
    {
        return get< int >(0x00000A20);
    }

    bool& m_bMaintainSequenceTransition()
    {
        return *(bool*)((uintptr_t)this + 0x9F0);
    }

    uint32_t& m_nLastSkipFramecount()
    {
    ...
    {
        return *(float*)(uintptr_t(this) + netvars::get().get_offset(crypt_str("CBaseAnimating"), crypt_str("m_flPlaybackRate")));
    }
    int& m_iMoveState()
    {
        static int m_iMoveState = netvars::get().get_offset(crypt_str("CCSPlayer"), crypt_str("m_iMoveState"));

        return *(int*)(uintptr_t(this) + m_iMoveState);
    }

    void*& m_pInverseKinematics()
    {
        return *(void**)((uintptr_t)this + 0x2670);
    }

    __forceinline void RunPostThink() {
        return call_virtual< void(__thiscall*)(decltype(this)) >(this, RUNPOSTTHINK)(this);
    }
    bool setup_local_bones(matrix3x4_t* matrix, int mask);
...
public:
    ...
    char bUnknown; //0x4
    char pad2[87];
    weapon_t* m_pWeaponLast; //0x5C
    player_t* m_pBasePlayer; //0x60
    weapon_t* weapon; //0x64
    ...
    float m_flSpeedAsPortionOfWalkTopSpeed; //0xF8 //from 0 to 2. something  is 1 when walking, 2.something when running, 0.653 when crouch walking
