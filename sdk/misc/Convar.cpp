...
public:
	virtual bool RegisterConCommandBase(ConCommandBase *pVar)
	{
		...
		m_cvar()->RegisterConCommand(pVar);
		return true;
	}
};
...
{
	if (!m_cvar() || s_bRegistered)
		return;

	assert(s_nDLLIdentifier < 0);
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = m_cvar()->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
	...
	assert(s_nDLLIdentifier >= 0);
	m_cvar()->UnregisterConCommands(s_nDLLIdentifier);
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
{
	if (m_cvar()) {
		m_cvar()->UnregisterConCommand(this);
	}
}
...
const char *ConVar::GetBaseName(void) const //-V524
{
	return m_pParent->m_pszName;
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
	...
	memcpy(pszOldValue, m_Value.m_pszString, m_Value.m_StringLength); //-V595

	int len = strlen(tempVal) + 1;

	if (len > m_Value.m_StringLength) {
		if (m_Value.m_pszString) { //-V809
			delete[] m_Value.m_pszString;
		}
	...
	if (m_cvar())
		m_cvar()->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
