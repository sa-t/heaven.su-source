...
bool weapon_t::is_pistol()
{
	...
	return weapon_id == WEAPON_ELITE || weapon_id == WEAPON_FIVESEVEN || weapon_id == WEAPON_P250 ||
...
void player_t::modify_eye_position(Vector& eye_position)
{
	...
	if (!local_animations::get().local_data.prediction_animstate->m_bLanding && local_animations::get().local_data.prediction_animstate->m_flAnimDuckAmount <= 0.0f)
		return;

	local_animations::get().local_data.prediction_animstate->m_pBasePlayer = this;
	...
	auto head_bone = lookup_bone(local_animations::get().local_data.prediction_animstate->m_pBasePlayer, crypt_str("head_0"));
...
void player_t::invalidate_bone_cache()
{
	...
	static auto m_InvalidateBoneCache = (util::FindSignature(crypt_str("client.dll"), crypt_str("80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81")));

	int iModelBoneCounter = **(int**)(((DWORD)(m_InvalidateBoneCache)) + 10);
...
bool player_t::physics_run_think(int index)//FIX
{
	static auto physics_run_think_fn = reinterpret_cast <bool(__thiscall*)(void*, int)> (util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 83 EC 10 53 56 57 8B F9 8B")));
...
bool player_t::setup_bones_fixed(matrix3x4_t* matrix, int mask)
{
	...
	if (previous_weapon)
		animstate->m_pWeaponLast = animstate->m_pWeapon;
...
bool player_t::setup_local_bones(matrix3x4_t* matrix, int mask)
{
	if (!this) //-V704
		return false;
