	...
	if (type && path.size() > 1)
	{
		...
		for (auto it = path.begin(); it != path.end(); ++it)
		{
			if (math::world_to_screen(prev, nadeStart) && math::world_to_screen(*it, nadeEnd))
				if (hastarget)
					render::get().line((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, g_cfg.esp.grenade_prediction_color_can_hit);
			if (!hastarget)
				render::get().line((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, g_cfg.esp.grenade_prediction_color);
		...
		*/
		...
		static auto alpha = 0.2f;

		Vector endpos = path[path.size() - 1];
		if (alpha > 0.1) {
			render::get().Draw3DCircleGradient(endpos, 2, Color(255, 0, 0, 155), alpha);
		}
		if (hastarget)
		{
			render::get().Draw3DCircleGradient(endpos, 33, g_cfg.esp.grenade_prediction_color_can_hit, alpha);
		}
		if (!hastarget)
		{
			render::get().Draw3DCircleGradient(endpos, 33, g_cfg.esp.grenade_prediction_color, alpha);
		}
		/*
		if (weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
			render::get().Draw3DCircle(endpos, 100, Color(240, 240, 240, 155));
		else if (weapon->m_iItemDefinitionIndex() == WEAPON_MOLOTOV || weapon->m_iItemDefinitionIndex() == WEAPON_INCGRENADE)
			render::get().Draw3DCircle(endpos, 100, Color(240, 240, 240, 155));
		if (weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE)
			render::get().Draw3DCircle(endpos, 50, Color(240, 240, 240, 155));
		*/
	}
}
