...
namespace ImGui
{
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f");     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
    ...
    // Tab Bars, Tabs
    IMGUI_API bool          tab(const char* icoid, const char* icon, bool selected, const ImVec2& icon_pos = {0,0});
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_RoundCornersBottomLeft = 1 << 3,
    ImDrawFlags_RoundCornersBottomRight = 1 << 3,
...
struct ImGuiStyle
{
    ...
    float ssadad;                           // Maximum error (in pixels) allowed when using AddCircle()/AddCircleFilled() or drawing rounded corner rectangles with no explicit segment count specified. Decrease for higher quality but more geometry.
