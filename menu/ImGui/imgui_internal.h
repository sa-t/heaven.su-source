...
struct IMGUI_API ImGuiWindow
{
    ...
    bool                    SkipItems;  
    ImVec2                  ContentSizeIdeal;// Set when items can safely be all clipped (e.g. window not visible or collapsed)
...
namespace ImGui
{
    ...
    //menu
    IMGUI_API bool          PlusButton(const char* label, bool type = false, const ImVec2& size_arg = ImVec2(0, 0), const char* uniq_id = "", ImColor col = ImColor(74 / 255.f, 165 / 255.f, 218 / 255.f), bool config = false);
    IMGUI_API ImVec2        CalcWindowNextAutoFitSize(ImGuiWindow* window);
    IMGUI_API bool subTabs(const char* label, bool selected, float sizex, int cout, bool reverse);
    ...
    IMGUI_API bool          KeybindButton(const char* label, const char* keylabel, const char* unique_id, bool clicked = false, ImGuiButtonFlags flags = 0);
    ...
    IMGUI_API bool          SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImRect* out_grab_bb, int* percent = NULL);
    ...
    template<typename T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API bool  SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, T* v, T v_min, T v_max, const char* format, ImRect* out_grab_bb, int* percent = NULL);
    template<typename T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API float ScaleRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max);
    template<typename T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API T     ScaleValueFromRatioT(ImGuiDataType data_type, float t, T v_min, T v_max);
