...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_act = ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, 1.f);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 icon_act = ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, 1.f);
...
bool ImGui::KeybindButton(const char* label, const char* keylabel, const char* unique_id, bool clicked, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 size = CalcItemSize({ 0,0 }, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    const ImRect total_bb(pos, pos + ImVec2(w, 20 + 10));
    window->DrawList->AddRectFilled({ total_bb.Min.x, total_bb.Max.y - 1 }, total_bb.Max, GetColorU32(ImGuiCol_Border));

    const ImRect bb(pos + ImVec2(w - size.x, 0), window->DC.CursorPos + ImVec2(w, 20 - 1));
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
    ...
    RenderText({ total_bb.Min.x, total_bb.Max.y - label_size.y - 13 }, keylabel);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, it_circle->second.alpha)), 60);
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size) - 16, inner_rect.Min.y, outer_rect.Max.x - 16, inner_rect.Max.y - 15);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, 1.f);
    ImVec4 hover_act = ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, 1.f);
    ...
    window->DrawList->AddRect(bb_frame.Min, bb_frame.Max, GetColorU32(ImGuiCol_Border), window->WindowRounding, rounding_corners);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect frame_bb(pos + ImVec2(w - square_sz, 0), window->DC.CursorPos + ImVec2(w, square_sz - 1));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(15, 0), window->DC.CursorPos + ImVec2(window->Size.x - 15, ImMin(24.f, label_size.y + style.FramePadding.y * 2.0f + 5)));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(22, 22, 22, int(255 * GetStyle().Alpha)), 4);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(30, 30, 30, int(255 * GetStyle().Alpha)), 4);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x) {
            window->DrawList->AddLine(ImVec2(value_x2 + 4, frame_bb.Min.y + 10), ImVec2(value_x2 + 8, frame_bb.Min.y + 14), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));
            window->DrawList->AddLine(ImVec2(value_x2 + 8, frame_bb.Min.y + 14), ImVec2(value_x2 + 12, frame_bb.Min.y + 9), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));

            window->DrawList->AddLine(ImVec2(value_x2 + 3, frame_bb.Min.y + 10), ImVec2(value_x2 + 8, frame_bb.Min.y + 15), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));
            window->DrawList->AddLine(ImVec2(value_x2 + 8, frame_bb.Min.y + 15), ImVec2(value_x2 + 13, frame_bb.Min.y + 9), ImColor(80, 80, 80, int(255 * GetStyle().Alpha)));
        }
    ...
    {
        ImVec2 preview_pos = frame_bb.Min + ImVec2(style.FramePadding.x, (10.5 - CalcTextSize(preview_value).y / 2));
        if (g.LogEnabled)
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 30, 0.0f), ImVec2(window->Size.x - 30, it_open->second * CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupBorderSize, 1);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImVec4 hover_act = ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(142 / 255.f, 169 / 255.f, 248 / 255.f, 0.4f);;
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImRect* out_grab_bb, int* percent)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            if (g.ActiveIdIsJustActivated)
            {
                g.DragCurrentAccum = 0.0f; // Reset any stored nav delta upon activation
                g.DragCurrentAccumDirty = false;
            }
            ...
            if (input_delta != 0.0f)
            {
                ...
                g.DragCurrentAccum += input_delta;
                g.DragCurrentAccumDirty = true;
            }

            float delta = g.DragCurrentAccum;
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated)
            {
                ClearActiveID();
            }
            else if (g.DragCurrentAccumDirty)
            {
                ...
                {
                    set_new_value = false;
                    g.DragCurrentAccum = 0.0f; // If pushing up against the limits, don't continue to accumulate
                }
                else
                {
                    set_new_value = true;
                    ...
                    if (delta > 0)
                        g.DragCurrentAccum -= ImMin(new_clicked_t - old_clicked_t, delta);
                    else
                        g.DragCurrentAccum -= ImMax(new_clicked_t - old_clicked_t, delta);
                }
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImRect* out_grab_bb, int* percent)
{
    switch (data_type)
    {
    case ImGuiDataType_S8: { ImS32 v32 = (ImS32) * (ImS8*)p_v;  bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min, *(const ImS8*)p_max, format, out_grab_bb, percent); if (r) *(ImS8*)p_v = (ImS8)v32;  return r; }
    case ImGuiDataType_U8: { ImU32 v32 = (ImU32) * (ImU8*)p_v;  bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min, *(const ImU8*)p_max, format, out_grab_bb, percent); if (r) *(ImU8*)p_v = (ImU8)v32;  return r; }
    case ImGuiDataType_S16: { ImS32 v32 = (ImS32) * (ImS16*)p_v; bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)p_min, *(const ImS16*)p_max, format, out_grab_bb, percent); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_U16: { ImU32 v32 = (ImU32) * (ImU16*)p_v; bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, out_grab_bb, percent); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_S32:
        IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
        return SliderBehaviorT<ImS32, ImS32, float >(bb, id, data_type, (ImS32*)p_v, *(const ImS32*)p_min, *(const ImS32*)p_max, format, out_grab_bb, percent);
    case ImGuiDataType_U32:
        IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT<ImU32, ImS32, float >(bb, id, data_type, (ImU32*)p_v, *(const ImU32*)p_min, *(const ImU32*)p_max, format, out_grab_bb, percent);
    case ImGuiDataType_S64:
        IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
        return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v, *(const ImS64*)p_min, *(const ImS64*)p_max, format, out_grab_bb, percent);
    case ImGuiDataType_U64:
        IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
        return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v, *(const ImU64*)p_min, *(const ImU64*)p_max, format, out_grab_bb, percent);
    case ImGuiDataType_Float:
        IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
        return SliderBehaviorT<float, float, float >(bb, id, data_type, (float*)p_v, *(const float*)p_min, *(const float*)p_max, format, out_grab_bb, percent);
    case ImGuiDataType_Double:
        IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
        return SliderBehaviorT<double, double, double>(bb, id, data_type, (double*)p_v, *(const double*)p_min, *(const double*)p_max, format, out_grab_bb, percent);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format)
{
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, &grab_bb, &percent);
    if (value_changed)
        MarkItemEdited(id);
    ...
    const ImRect slider_bb({ frame_bb.Min.x + value->second - 8, frame_bb.GetCenter().y - 8 }, { frame_bb.Min.x + value->second + 8, frame_bb.GetCenter().y + 9 });
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= SliderScalar("", data_type, v, v_min, v_max, format);
        PopID();
        PopItemWidth();
...
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format);
}

bool ImGui::SliderFloat3(const char* label, float v[3], float v_min, float v_max, const char* format)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format);
}

bool ImGui::SliderFloat4(const char* label, float v[4], float v_min, float v_max, const char* format)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min, &v_max, format);
}
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max, const char* format)
{
    ...
    bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, format);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGuiLastItemData item_data_backup;//ImGuiItemHoveredDataBackup
    if (is_multiline)
    {
    ...
    const bool input_requested_by_nav = (g.ActiveId != id) && ((g.NavInputId == id) || (g.NavActivateId == id && g.NavInputSource == ImGuiInputSource_NavKeyboard));
    ...
    if (g.ActiveId == id)
    {
        ...
        state->UserFlags = ImGuiInputTextFlags_None;
    }
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts)
{
    ...
    if (flags & ImGuiColorEditFlags_NoInputs)
        flags = (flags & (~ImGuiColorEditFlags__DisplayMask)) | ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoInputs;
    ...
    IM_ASSERT(ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DisplayMask));
    ...
    it_open->second.mult = math::clamp(it_open->second.mult + 0.01f + (it_open->second.mult * 0.03f) * GetIO().DeltaTime * (it_open->second.play ? 1.f : -1.f), 0.01f, 1.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, { 20, 20 }))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(window->DC.LastItemRect.GetBL() + ImVec2(0, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupContextItem("context");

        GetStyle().ssadad = min(it_open->second.mult * 3.2, 1.f);
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(33 / 255.f, 33 / 255.f, 33 / 255.f, it_open->second.mult * 0.8f));
        if (BeginPopup("picker", ImGuiWindowFlags_::ImGuiWindowFlags_NoMove))
        {
            ...
            SetNextItemWidth(square_sz * 9.0f);
            value_changed |= ColorPicker4("##picker", colors[config_name], picker_flags, &g.ColorPickerRef.x);
            EndPopup();
        }
        else
    ...
    if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget())
    {
        ...
        {
            memcpy((float*)colors[config_name], payload->Data, sizeof(float) * 3);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImGui::SetCursorPosX(window->Size.x - 25);
        ...
        PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupRounding, 4);
        PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_FramePadding, ImVec2(1, 1));
        PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_WindowPadding, ImVec2(1, 1));
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(33 / 255.f, 33 / 255.f, 33 / 255.f, it_open->second.mult * 0.8f));
        if (BeginPopup("picker", ImGuiWindowFlags_::ImGuiWindowFlags_NoMove))
        {
            picker_active_window = g.CurrentWindow;
            ...
            it_open->second.play = false;
        PopStyleVar(5);    PopStyleColor(1);
    }

    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        const float text_offset_x = w_button;
...
bool ImGui::subTabs(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    auto al = float(ImGui::GetStyle().Alpha);
    ...
    it_filled->second = ImClamp(it_filled->second + (2.15f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.0f)), 0.f, 1.f);
    it_filled->second *= min(GetStyle().Alpha * 1.5, 1.f);
    float t = it_filled->second;

    ImVec4 col = ImLerp(ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, 0.7f * al }, ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, 0.5f * al }, t * al);
    ImVec4 col2 = ImLerp(ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, 0.4f * al }, ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, 0.5f * al }, t * al);
    ImVec4 col3 = ImLerp(ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, 0.4f * al }, ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, 1.f * al }, t * al);
    if (hovered && !selected)
    {
        col = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 0.5 * al);
        col2 = ImVec4(200 / 255.f, 200 / 255.f, 200 / 255.f, 0.5 * al);
    }
    if (it_filled->second > 0)
        if (!reverse)
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + size.x * min(t * 2, 1), bb.Min.y + 2 * min(t, 1)), ImColor(g_cfg.misc.menu_color.r(), g_cfg.misc.menu_color.g(), g_cfg.misc.menu_color.b(), (int)((255 * t) * al)), 12, ImDrawCornerFlags_Bot);

    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size.x / 2 - label_size.x / 2), bb.Min.y + float((25 + 5.f * min(t * 2, 1)) / 2 - label_size.y / 2)), ImColor(col.x, col.y, col.z, col.w * al), label);

    return pressed;
}

bool ImGui::PlusButton(const char* label, bool type, const ImVec2& size_arg, const char* uniq_id, ImColor col, bool config)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(type ? label : uniq_id);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    auto flags = 0;
    ...
    it_hover->second = ImClamp(it_hover->second + (5.f * ImGui::GetIO().DeltaTime * (hovered || IsPopupOpen(uniq_id) ? 1.f : -1.f)), 0.0f, 1.f);
    auto d0nor = col;
    const ImVec4 text_act = ImVec4(225 / 255.f, 225 / 255.f, 225 / 255.f, int(100 + (155 * it_hover->second)) / 255.f);
    const ImVec4 icon_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    ImVec4 colr = ImLerp(ImVec4{ 1.f, 1.f, 1.f, GetStyle().Alpha }, ImVec4{ 34 / 255.f, 33 / 255.f, 39 / 255.f, GetStyle().Alpha }, it_hover->second);
    if (config)
        RenderFrame(bb.Min, bb.Max, ImColor(25 / 255.f, 25 / 255.f, 25 / 225.f, 225.f - it_hover->second), true, 24 - (20 * it_hover->second));
    RenderFrame(bb.Min, bb.Max, ImColor(col.Value.x, col.Value.y, col.Value.z, it_hover->second), true, 24 - (20 * it_hover->second));
    if (!type)
    ...
    static std::map<ImGuiID, popup_anims> open_animation;
    auto it_open = open_animation.find(ImGui::GetCurrentWindow()->GetID(uniq_id));
    if (it_open == open_animation.end())
    {
        open_animation.insert({ ImGui::GetCurrentWindow()->GetID(uniq_id), {false, 0.f} });
        it_open = open_animation.find(ImGui::GetCurrentWindow()->GetID(uniq_id));
    }
    it_open->second.mult = math::clamp(it_open->second.mult + 0.03f + (it_open->second.mult * 0.03f) * GetIO().DeltaTime * (it_open->second.play ? 1.f : -1.f), 0.01f, 1.f);
    if (!it_open->second.play)
        it_open->second.mult = 0.01f;
    g_ctx.gui.open_pop = it_open->second.play;
    g_ctx.gui.pop_anim = it_open->second.mult;
    if (!type)
    {
        if (ImGui::GetIO().MouseClicked[0] && hovered)
        {
            it_open->second.play = true;
            ImGui::OpenPopup(uniq_id);
            ImGui::SetNextWindowPos(window->DC.LastItemRect.GetBL() + ImVec2(0, ImGui::GetStyle().ItemSpacing.y - 2));
        }
    }
    if (!IsPopupOpen(uniq_id))
        it_open->second.play = false;
